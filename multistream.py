...
class Stream_b_only (nn.Module):

    def __init__(self , model, concat_dim, num_classes, raft_parameters_path, device, raft_iters = 12 , trainable = True):
      """
      a model for training the streams only.

      args:
      ==================================
        model (str) :  the chosen model to train the stream b on.
        trainable (bool) : to choose whether to train the pretrained model of bor not.
        concat_dim : the output dim of the pretrained model. the dim at which the output of the streams will be concated together.
        num_classes : the number of classes of the main dataset.
        device : the device at which to train the model.
        raft_parameters_path : the path of the pretrained raft parameters.
        raft_iters : number of iterations of the raft.
      ...
      self.stream_b_model = Stream_b(model, concat_dim, raft_parameters_path, device, raft_iters , trainable)

      self.concat = Concate(n_streams = 1, in_dim = concat_dim, out_dim = concat_dim)
    ...
    def forward(self, x):
      ...
      out = self.concat([stream_b_out]) #(b  , concat_dim)
